...
const pairs = [
  ...
  // Golden + broken fixtures. Mirrored into api/_lib/golden-cases/ so the
  // daily regression cron (api/_lib/contract-safety-checks.js) can require
  // them from inside the serverless bundle without dragging scripts/ in.
  // Heath's originals under scripts/ remain the source of truth and are never
  // regenerated by an agent.
  ...['conventional', 'cash', 'fha', 'va', 'seller', 'assumption'].map((name) => [
