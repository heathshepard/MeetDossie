...
function cropAdvice(answer) {
  ...
  return [
    ...
    '- The composer reads **"tap the mic to start a voice call"** — visible chrome, fine to frame,',
    '  never tap it. The **"🔊 Speaking…"** pill (auto-TTS on every answer) is now suppressed AT',
    '  CAPTURE TIME by the recorder itself — `?captureMode=1` on the app URL plus a Playwright',
    '  route abort on `/api/speak` (record-dossie-shortform-frames.js) — so it should not appear',
    '  in frames.json at all for this take. This note stays as a compositor-side backstop only:',
    '  if an older capture (pre this fix) is ever re-composited, still never punch in on, frame,',
    '  or narrate the mic / speaking affordance. D1 is a TYPED question and a TEXT answer;',
    '  capability #12 (spoken voice I/O) is PARTIAL/UNVERIFIED.',
