...
async function main() {
  ...
  // 4. Cadence math — HOT WINDOW scheme, TIGHTENED 2026-09-16 for the
  // auto-reply-with-veto 1-hour SLA (supabase/migrations/20260916_auto_reply_veto.sql):
  // first 48h: first pass at +15min then every 15min; after 48h: every 3
  ...
  assert.strictEqual(isDue({ ...post, harvest_count: 0 }, t0 + 20 * 60e3), true, 'first pass due at +15min (hot window)');
  const hot1 = new Date(t0 + 60 * 60e3).toISOString();
  assert.strictEqual(isDue({ ...post, harvest_count: 1, last_harvested_at: hot1 }, t0 + 70 * 60e3), false, 'hot window: not due 10 min after last pass');
  assert.strictEqual(isDue({ ...post, harvest_count: 1, last_harvested_at: hot1 }, t0 + 80 * 60e3), true, 'hot window: due 20 min after last pass');
