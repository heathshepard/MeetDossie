-- =============================================================================
-- Fix wrong Team seat-overage price baked into create_org_with_founder's
-- SQL default (p_seat_price_cents DEFAULT 7900 = $79/seat).
--
-- Per CLAUDE.md Section 5, Team overage is $35/seat (3500 cents) above the
-- 3 included seats, max 8 total. The 7900 value was a placeholder from the
-- original Phase 4 migration (20260620120000) that never got corrected when
-- real Team pricing was locked in. Every real caller (api/team/create-org.js,
-- api/_lib/team-org.js) already passes p_seat_price_cents explicitly, so this
-- default was dead in practice — but it's still wrong, and a future direct
-- RPC call that omits the param would silently charge the wrong number.
--
-- This migration only changes the DEFAULT; the function body is otherwise
-- byte-for-byte identical to 20260620120000_multitenant_phase4_rpcs_and_solo_upgrade.sql
-- (CREATE OR REPLACE FUNCTION requires the full definition, not just the
-- changed line).
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.create_org_with_founder(
  ...
  p_seat_price_cents INT DEFAULT 3500,
...
COMMENT ON FUNCTION public.create_org_with_founder IS 'Atomic org creation + founder seating. DOD-O-1, DOD-O-2, DOD-S-7. Seat price default fixed 2026-08-23 (was $79/seat, correct is $35/seat per CLAUDE.md Section 5).';
