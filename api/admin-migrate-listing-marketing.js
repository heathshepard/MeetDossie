...
// One-time migration: create listing_marketing_status + listing_marketing_rotation
// (ongoing daily marketing pipeline for Heath's active real estate listings).
// DDL isn't reachable through PostgREST -- runs directly against Postgres via
// api/_lib/pg-admin.js (POSTGRES_URL_NON_POOLING), same pattern as
// api/admin-migrate-comment-opportunities.js. Local POSTGRES_* env is
// write-only ([SENSITIVE]) so this can only run from a deployed environment:
//   curl -H "Authorization: Bearer $CRON_SECRET" \
//     https://<preview-or-prod>/api/admin-migrate-listing-marketing
//
// Mirrors supabase/migrations/20260910b_listing_marketing_pipeline.sql.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, message: 'listing_marketing_status + listing_marketing_rotation ready' });
  } catch (err) {
    console.error('[admin-migrate-listing-marketing] Fatal:', err && err.message);
