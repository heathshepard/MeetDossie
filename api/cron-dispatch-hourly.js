...
// vercel.json with its OWN cron entry at schedule "0 * * * *". Merged
...
//   - /api/cron-mission-watchdog
//   - /api/cron-unsubscribe-spike-monitor
//
...
const HANDLERS = [
  { name: 'cron-mission-watchdog', mod: require('./cron-mission-watchdog.js') },
  { name: 'cron-unsubscribe-spike-monitor', mod: require('./cron-unsubscribe-spike-monitor.js') },
];
...
module.exports = async function handler(req, res) {
  ...
  return res.status(anyFail ? 207 : 200).json({
    ok: !anyFail,
    dispatcher: 'cron-dispatch-hourly',
    schedule: '0 * * * *',
