...
//
// Mirrors api/cron-email-to-dossier.js's entitlement check (same table,
// same "active subscription" condition, same HEATH_KW_USER_ID dogfood
// bypass) so this never drifts from what the cron actually gates on.
//
// 2026-09-16 CARTER — this file was written before the "Reply Monitoring"
// add-on was renamed "Email Integration" and its column renamed
// subscriptions.reply_monitoring_enabled -> email_integration_enabled
// (see api/admin-migrate-email-integration-addon.js, commit bf1f5273, and
// the current column list in api/cron-email-to-dossier.js). This file was
// never updated to match, was never committed, and querying the old column
// name against the live schema would 400. Fixed to read
// email_integration_enabled. Route path/name kept as-is since the client
// (dossie-app.jsx) already calls GET /api/reply-monitoring-status.
//
...
// email_integration_enabled=true for any OTHER account's subscription row
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const subRes = await supa(
      'subscriptions?select=email_integration_enabled&user_id=eq.' +
    ...
    const enabled = !!(Array.isArray(rows) && rows[0] && rows[0].email_integration_enabled === true);
