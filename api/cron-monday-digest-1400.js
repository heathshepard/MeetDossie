...
// api/cron-monday-digest-1400.js
// Consolidated dispatcher — all three crons below already ran at 0 14 * * 1
// (Monday 14:00 UTC) as separate Vercel cron entries. Freed for the
...
//
// Was: /api/cron-weekly-scorecard, /api/cron-growth-alerts,
//      /api/cron-fb-unlock-alerts
...
module.exports = withTelemetry('cron-monday-digest-1400', async function handler(req, res) {
  if (!isAuthorizedCronCaller(req)) {
    return res.status(401).json({ ok: false, error: 'Unauthorized' });
  }

  const results = await fanout([
    '/api/cron-weekly-scorecard',
    '/api/cron-growth-alerts',
    '/api/cron-fb-unlock-alerts',
  ]);
