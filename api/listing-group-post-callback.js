...
async function handleListingGroupPostCallback(action, postId, deps) {
  ...
  if (action === 'lst_approve') {
    ...
    if (!patch.ok) {
      // Never collapse a real write failure into "Already handled" -- see
      // api/group5-post-callback.js for the 2026-09-11 root cause
      // (auto_post_at missing from the live schema since 2026-06-10).
      const errText = patch.data?.message || `HTTP ${patch.status}`;
      console.error(`[listing-group-post-callback] Approve PATCH failed for ${postId}:`, errText);
      if (chatId && messageId) {
  ...
  if (action === 'lst_skip') {
    ...
    if (!patch.ok) {
      const errText = patch.data?.message || `HTTP ${patch.status}`;
      console.error(`[listing-group-post-callback] Skip PATCH failed for ${postId}:`, errText);
      if (chatId && messageId) {
