...
const {
  MAP_VERSION: FIELD_MAP_VERSION,
  resolveFieldValue,
} = require('./_lib/trec-20-19-transaction-field-map');
...
async function handlePhase1Init({ req, res, userId, transactionId }) {
  ...
  const draftsForForm = (txn.contract_field_drafts && typeof txn.contract_field_drafts === 'object')
    ...
    : {};
  //
  // 2026-08-16 CARTER — the prefill bug. The `hasOwnProperty(txn, canonical)`
  // branch below treated a TREC field key as a literal `transactions` column
  // name. Only 10 of the 180 agent-fillable keys on 20-19 happen to collide
  // with a real column, so a dossier holding sale price, earnest money, option
  // fee and title company still opened the editor almost entirely blank —
  // the "filled 2 of 31 required fields" report. The semantic map now resolves
  // the key properly, and the literal-column lookup is kept only as a
  // last-resort fallback for the handful of keys that genuinely are columns.
  const enriched = [];
  let unmappedCount = 0;
  let recordCount = 0;
  let derivedCount = 0;
  for (const raw of fable5Fields) {
    ...
    let provenance = null;
    let provenanceBasis = null;
    if (canonical && Object.prototype.hasOwnProperty.call(draftsForForm, canonical)) {
      // Last thing the agent typed always wins.
      currentValue = draftsForForm[canonical];
      provenance = 'agent';
    } else if (canonical) {
      const resolved = resolveFieldValue(canonical, txn);
      if (resolved) {
        currentValue = resolved.value;
        provenance = resolved.provenance;
        provenanceBasis = resolved.basis;
        if (resolved.provenance === 'derived') derivedCount += 1;
        else recordCount += 1;
      } else if (Object.prototype.hasOwnProperty.call(txn, canonical)) {
        currentValue = txn[canonical];
        provenance = 'record';
        provenanceBasis = canonical;
        if (currentValue != null && String(currentValue).trim() !== '') recordCount += 1;
      }
    }
    const field = enrichFable5Field(raw, currentValue, { provenance, provenanceBasis });
    if (!field) continue;
  ...
  return res.status(200).json({
    ...
    totals: { total, required, filled, missing },
    unmappedCount,
    // Which semantic map produced these values. Pin this onto the
    // signature_request so an executed contract can always be traced back to
    // the map version that filled it.
    fieldMapVersion: FIELD_MAP_VERSION,
    provenanceSummary: {
      from_record: recordCount,
