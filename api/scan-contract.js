...
- Paragraph 3 SALES PRICE: 3A cash portion, 3B sum financed, 3C TOTAL sales price.
  CRITICAL — SALES PRICE WITH VERIFICATION: Read all three values from Paragraph 3:
  1. Extract the dollar amount from line 3A (cash portion)
  2. Extract the dollar amount from line 3B (sum financed)
  3. Extract the dollar amount from line 3C (labeled "Sales Price (Sum of A and B)")
  4. Calculate: 3A + 3B = expected total
  5. If the stated 3C value does NOT match your calculated sum (3A + 3B), use the calculated sum as the correct salePrice. OCR errors can cause 3C to be misread — the math is always correct.
...
- earnestMoneyReceiptDate — EARNEST MONEY RECEIPT BLOCK: TREC 20-x contracts end with a signature-block page titled "OPTION FEE RECEIPT" / "EARNEST MONEY RECEIPT" / "CONTRACT RECEIPT" / "ADDITIONAL EARNEST MONEY RECEIPT" (usually the very last page of the PDF, filled in by hand/DocuSign by the title company's escrow officer AFTER the contract is signed, not part of the numbered paragraphs 1-23). Find the "EARNEST MONEY RECEIPT" section specifically (do not confuse with "OPTION FEE RECEIPT" above it or "CONTRACT RECEIPT" below it — they are three separate boxes on the same page, each with their OWN handwritten date that CAN be different from the other two, though escrow officers commonly fill all three boxes in one sitting so the dates frequently match). It reads "Receipt of $[amount] earnest money in the form of [check#] is acknowledged" followed by "Escrow Agent / Received by / Email Address / Date/Time" signature line. Extract the date from THAT SPECIFIC "Date/Time" field, directly under "EARNEST MONEY RECEIPT" (e.g. "8/6/26" -> "2026-08-06") into earnestMoneyReceiptDate.
  ...
  CRITICAL — READ THE DIGIT SHAPES CAREFULLY, THIS HANDWRITING IS EASY TO MISREAD: a single slanted "1" immediately followed by a "/" is commonly misread as "11" (two strokes) — look for whether the SECOND mark is actually a "/" (a thin diagonal slash, distinct stroke angle from a digit) before concluding the month is "11". A "3" is commonly misread as "2" or "0" — trace whether the digit has one bump (a "2"'s curve into a flat base) or two stacked bumps (a "3"). Confirmed real misreads on this exact failure mode: "1/30/23" -> wrongly read as "11/20/23" and separately, on a re-scan of the SAME document, as "11/2/23" — two different wrong answers from the same handwriting, which is itself a sign this date is genuinely easy to misread and needs extra care, not a quick first impression. Do not pattern-match to "a plausible-looking date"; trace each individual character left to right (digit, slash-or-digit, digit, digit, slash, digit, digit) before deciding what the month/day/year actually are.
  SELF-CHECK AGAINST THE DELIVERY DEADLINE: you already extract paragraph5.earnestMoneyDeadlineDays (days after the Effective Date the earnest money is due) and contractEffectiveDate elsewhere in this same extraction. Earnest money is almost always actually delivered within about 0-30 days of contractEffectiveDate — title companies acknowledge receipt promptly, not months later. Before finalizing earnestMoneyReceiptDate, sanity-check it against contractEffectiveDate + earnestMoneyDeadlineDays: if your reading puts the receipt date more than ~45 days after the Effective Date (or before it), that is a strong signal you misread a digit (most often the "1 vs 11" or "3 vs 2" confusion above) — go back and re-trace the handwritten characters rather than accepting an implausible date at face value.
  CROSS-CHECK: read the handwritten "Date" fields in the neighboring OPTION FEE RECEIPT box (above) and CONTRACT RECEIPT box (below) on the same page too. Escrow officers very often fill all three boxes in one sitting, so those two dates are frequently IDENTICAL to the correct EARNEST MONEY RECEIPT date — if your first read of the EARNEST MONEY RECEIPT date disagrees with a clearer, more confident read of either neighboring date, that is a signal to re-examine your first read, not proof the dates differ. Only keep them as genuinely different if you can clearly and confidently read distinct digits in each box. Never copy a neighboring box's date when the EARNEST MONEY RECEIPT box's own Date/Time field is truly blank — this cross-check is for resolving an ambiguous/hard-to-read date already present in the box, not for filling in a missing one. NOTE: because all three boxes are usually written by the same hand in one sitting, they can share the SAME misread too (the "1/30" vs "11/2"/"11/20" confusion above has been seen consistently across all three boxes on the same page) — agreement between boxes increases confidence you read the SAME thing correctly, but does not by itself prove that shared reading is the CORRECT one. The delivery-deadline self-check above is the stronger signal when the two disagree.
...
3. Convert all dates to yyyy-MM-dd format. If only month/day is given without a year, use null and add a warning.
...
async function scanContract(pdfBase64) {
  ...
  // Plausibility guard for earnestMoneyReceiptDate: earnest money is almost
  // always delivered within days-to-weeks of the Effective Date, never
  // months later. This does NOT null out or silently rewrite the value
  // (confirmed 2026-08-07: this exact field's handwritten date has been
  // independently misread two different wrong ways by the model already —
  // "11/20/23" and "11/2/23" for an actual "1/30/23" — a code-level digit
  // remap would be guessing which wrong reading to "fix" into which right
  // one, and could just as easily corrupt a genuinely-different real date on
  // another contract). Surface a warning instead so it's visible in the scan
  // review UI and Heath can confirm/correct it by eye, same as any other
  // low-confidence field.
  if (extracted.earnestMoneyReceiptDate && extracted.contractEffectiveDate) {
    const receiptMs = Date.parse(extracted.earnestMoneyReceiptDate + 'T00:00:00Z');
    const effectiveMs = Date.parse(extracted.contractEffectiveDate + 'T00:00:00Z');
    if (Number.isFinite(receiptMs) && Number.isFinite(effectiveMs)) {
      const daysDiff = (receiptMs - effectiveMs) / (24 * 60 * 60 * 1000);
      if (daysDiff < -3 || daysDiff > 45) {
        warnings.push(
          `Earnest money receipt date (${extracted.earnestMoneyReceiptDate}) is ${Math.round(daysDiff)} days from the contract Effective Date (${extracted.contractEffectiveDate}) — outside the normal 0-30 day delivery window. Handwritten receipt dates on this page are easy to misread (e.g. "1/30" vs "11/20"); double-check the "EM confirmed by title" date against the actual document before relying on it.`
        );
      }
    }
  }
