...
// import_email_attachments) and the member-memory tools (remember_preference/
// remember_fact). A single model turn may call tools from either group (or
// both, in sequence) before landing on whatever client-dispatchable action it
// concludes with; one shared conversation/tool-count loop is what keeps that
// composable — two independent loops each reconstructing `conversation` from
// scratch would silently drop whichever group's exchange ran first.
//
...
// inbox-resolve-loop.js's own reasoning) + 2 memory writes is enough for any
// turn that both files something and jots down a preference or two without
// giving a runaway loop room to walk either surface indefinitely.
const MAX_SERVER_TOOL_CALLS = 6;

const SERVER_TOOL_NAMES = new Set([...INBOX_TOOL_NAMES, ...MEMORY_TOOL_NAMES]);
...
async function runServerToolResolveLoop({
  ...
  while (calls < MAX_SERVER_TOOL_CALLS) {
    ...
    let toolResult;
    try {
      toolResult = INBOX_TOOL_NAMES.has(toolUse.name)
        ? await executeInboxTool(toolUse.name, toolUse.input || {}, { userId })
        : await executeMemoryTool(toolUse.name, toolUse.input || {}, { userId });
    } catch (err) {
      if (err instanceof InboxSecurityError || err instanceof MemorySecurityError) {
